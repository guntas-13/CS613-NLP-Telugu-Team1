    ...
    try:
        ...
        if main_div.find_all('p'):
            ...
            for p in main_div.find_all('p'):
                extracted_data += p.text + "\n"
        else:
            extracted_data = main_div.text
...
def process_all_links(df, output_folder, failed_csv_path):
    ...
    with ThreadPoolExecutor(max_workers=30) as executor:
        ...
        for i in range(len(df)):
            link = df["links"][i]
            link = "https://www.tupaki.com/" + link
...
csv_path = "./Tupaki-Guntas/TupakiLinks-Guntas.csv"

df = pd.read_csv(csv_path)

if __name__ == "__main__":
    output_folder = "../TeluguData/Tupaki"
    failed_csv_path = "./failed_links_teTupaki.csv"
